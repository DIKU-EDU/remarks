...
addPredifinedPropsLeaf p =
  Property ("Title", Lookup (0, Value "Title"))
    ...
    : Property ("TotalTasks", ArithFun Sum [Lookup (0, Value "TotalTasks")])
    : p
...
addPredifinedProps p sublen =
  Property ("Title", Lookup (0, Value "Title"))
    ...
    : Property ("TotalTasks", ArithFun Sum (map (\x -> Lookup (x, Value "TotalTasks")) [1 .. sublen]))
    : p
...
evalPropExp rj (ArithFun _ [pname]) propEnv
  | isLeafJ rj =
